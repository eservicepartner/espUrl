...
namespace Greenshot.Forms
{
   ...
   public partial class CaptureForm : Form, ICaptureHost
   {
      ...
      /// </summary>
      private void PrepareForCaptureWithFeedback(CaptureMode mode)
      {
         // Only init when CaptureMode.Window or CaptureMode.Region
         if (!CaptureMode.Window.Equals(mode) && !CaptureMode.Region.Equals(mode) && !CaptureMode.Video.Equals(mode))
         {
            return;
         }
      ...
      {
         if (MouseHelper.CheckKeyUpForFixing(e.KeyCode))
         {
            return;
         }
      }
      ...
      void PictureBoxPaint(object sender, PaintEventArgs e)
      {
         ...
         if (capture.CaptureDetails.CaptureMode == CaptureMode.Video)
         {
            ...
            {
               graphics.FillEllipse(brush, cursorPos.X + 20, cursorPos.Y + 20, 20, 20);
            }
         }
      }
      #endregion
